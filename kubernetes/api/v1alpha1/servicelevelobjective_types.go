...
type ServiceLevelObjectiveSpec struct {
	...
	// +optional
	// PreAggregation enables pre-computed hourly blocks for high-cardinality metrics.
	// When enabled, burn rate calculations use pre-aggregated data to reduce query load.
	PreAggregation *PreAggregationSpec `json:"preAggregation,omitempty"`
...
// PreAggregationSpec configures pre-computed hourly blocks for high-cardinality metrics.
...
type PreAggregationSpec struct {
	// +optional
	...
	// BlockWindow is the interval for pre-aggregation blocks.
	// Burn rate windows longer than this will use pre-aggregated data.
	// Valid time units are "s", "m", "h".
	BlockWindow string `json:"blockWindow,omitempty"`
}
...
func (in *ServiceLevelObjective) Internal() (slo.Objective, error) {
	...
	var preAggregation slo.PreAggregation
	if in.Spec.PreAggregation != nil {
		preAggregation.Enabled = in.Spec.PreAggregation.Enabled
		if in.Spec.PreAggregation.BlockWindow != "" {
			blockWindow, err := model.ParseDuration(in.Spec.PreAggregation.BlockWindow)
			if err != nil {
				return slo.Objective{}, fmt.Errorf("failed to parse pre-aggregation block window: %w", err)
			}
			preAggregation.BlockWindow = blockWindow
		}
	}
