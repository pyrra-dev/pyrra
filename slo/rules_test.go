...
func TestObjective_Burnrates(t *testing.T) {
	...
	for _, tc := range testcases {
		t.Run(tc.name, func(t *testing.T) {
			group, err := tc.slo.Burnrates("http://localhost:9090")
...
func TestObjective_PyrraURLAnnotation(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			wantURL:     "https://pyrra.example.com/objectives/monitoring-http-errors",
			wantPresent: true,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			burnrates, err := tt.objective.Burnrates(tt.pyrraURL)
			require.NoError(t, err)
