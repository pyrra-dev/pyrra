	...
	case Ratio:
		...
		for _, br := range burnrates {
			rules = append(rules, monitoringv1.Rule{
				...
				Expr:   intstr.FromString(o.BurnrateExpr(br)),
				Labels: ruleLabels,
			})
		}
...
// BurnratePreagg generates a burn rate expression using pre-aggregated block data.
// Instead of rate(raw_metric[window]), it uses sum_over_time(pre_aggregated_metric[window])
// which dramatically reduces the number of samples scanned for long windows.
//
// The pre-aggregated metrics are expected to be generated by BlockRules().
func (o Objective) BurnratePreagg(timerange time.Duration) string {
	sloName := o.Labels.Get(labels.MetricName)
	blockWindow := o.PreAggregation.EffectiveBlockWindow()

	switch o.IndicatorType() {
	case Ratio:
		// For ratio: sum_over_time(errors_block[window]) / sum_over_time(total_block[window])
		totalBlockName := preAggBlockName(o.Indicator.Ratio.Total.Name, blockWindow)
		errorsBlockName := preAggBlockErrorsName(o.Indicator.Ratio.Errors.Name, blockWindow)
		...
		if len(grouping) > 0 {
			groupByClause = fmt.Sprintf(" by (%s)", strings.Join(grouping, ", "))
		}

		windowStr := model.Duration(timerange).String()

		return fmt.Sprintf(
			`sum%s(sum_over_time(%s{slo="%s"}[%s])) / sum%s(sum_over_time(%s{slo="%s"}[%s]))`,
			groupByClause,
			errorsBlockName, sloName, windowStr,
			groupByClause,
			totalBlockName, sloName, windowStr,
		)

	case Latency:
		// For latency: (total - success) / total
		// = (sum_over_time(total_block[window]) - sum_over_time(success_block[window])) / sum_over_time(total_block[window])
		totalBlockName := preAggBlockName(o.Indicator.Latency.Total.Name, blockWindow)
		successBlockName := preAggBlockName(o.Indicator.Latency.Success.Name, blockWindow)
		...
		return fmt.Sprintf(
			`(sum%s(sum_over_time(%s{slo="%s"}[%s])) - sum%s(sum_over_time(%s{le="%s",slo="%s"}[%s]))) / sum%s(sum_over_time(%s{slo="%s"}[%s]))`,
			groupByClause,
			totalBlockName, sloName, windowStr,
			groupByClause,
			successBlockName, le, sloName, windowStr,
			groupByClause,
			totalBlockName, sloName, windowStr,
		)

	default:
		// For LatencyNative and BoolGauge, fall back to standard burn rate
		// These types are less common and can be enhanced later if needed
		return o.Burnrate(timerange)
	}
}

// ShouldUsePreagg returns true if the given timerange should use pre-aggregated data.
// Windows shorter than or equal to the block window use raw data for accuracy.
// Windows longer than the block window benefit from pre-aggregation.
func (o Objective) ShouldUsePreagg(timerange time.Duration) bool {
	if !o.PreAggregation.Enabled {
		return false
	}
	blockWindow := time.Duration(o.PreAggregation.EffectiveBlockWindow())
	// Use pre-aggregation for windows longer than the block window
	return timerange > blockWindow
}

// BurnrateExpr returns the appropriate burn rate expression for the given window.
// If pre-aggregation is enabled and the window is longer than the block window,
// it uses BurnratePreagg() which queries pre-aggregated data.
// Otherwise, it uses the standard Burnrate() which queries raw data.
func (o Objective) BurnrateExpr(timerange time.Duration) string {
	if o.ShouldUsePreagg(timerange) {
		return o.BurnratePreagg(timerange)
	}
	return o.Burnrate(timerange)
}
...
// preAggBlockName returns the name for a pre-aggregated total block recording rule.
// Example: http_requests:increase1h
func preAggBlockName(metric string, blockWindow model.Duration) string {
	metric = strings.TrimSuffix(metric, "_total")
	metric = strings.TrimSuffix(metric, "_count")
	metric = strings.TrimSuffix(metric, "_bucket")
	return fmt.Sprintf("%s:increase%s", metric, blockWindow)
}

// preAggBlockErrorsName returns the name for a pre-aggregated errors block recording rule.
// Example: http_requests:increase1h:errors
func preAggBlockErrorsName(metric string, blockWindow model.Duration) string {
	metric = strings.TrimSuffix(metric, "_total")
	metric = strings.TrimSuffix(metric, "_count")
	metric = strings.TrimSuffix(metric, "_bucket")
	return fmt.Sprintf("%s:increase%s:errors", metric, blockWindow)
}
...
// BlockRules generates pre-aggregation recording rules for high-cardinality metrics.
// These rules pre-compute increase() over fixed intervals (BlockWindow), allowing
// burn rate calculations to use sum_over_time() on pre-aggregated data instead of
// scanning raw time series.
//
// Generated rules:
//   - metric:increase<blockWindow> - pre-aggregated total requests
//   - metric:increase<blockWindow>:errors - pre-aggregated error requests
//
// This dramatically reduces query load for long SLO windows (e.g., 28d) on
// high-cardinality metrics by reducing samples from millions to hundreds.
func (o Objective) BlockRules() (monitoringv1.RuleGroup, error) {
	if !o.PreAggregation.Enabled {
	...
	increaseExpr := func() (parser.Expr, error) {
		return parser.ParseExpr(`sum by (grouping) (increase(metric{matchers="total"}[1s]))`)
	}
	...
	case Ratio:
		...
		objectiveReplacer{
			...
			matchers: o.Indicator.Ratio.Total.LabelMatchers,
			grouping: grouping,
			window:   time.Duration(blockWindow),
		}.replace(expr)

		rules = append(rules, monitoringv1.Rule{
			Record: preAggBlockName(o.Indicator.Ratio.Total.Name, blockWindow),
			Expr:   intstr.FromString(expr.String()),
			Labels: ruleLabels,
		})
		...
		rules = append(rules, monitoringv1.Rule{
			Record: preAggBlockErrorsName(o.Indicator.Ratio.Errors.Name, blockWindow),
			Expr:   intstr.FromString(expr.String()),
			Labels: ruleLabels,
		})
	...
	case Latency:
		...
		rules = append(rules, monitoringv1.Rule{
			Record: preAggBlockName(o.Indicator.Latency.Total.Name, blockWindow),
			Expr:   intstr.FromString(expr.String()),
			Labels: ruleLabels,
		})

		// Success (used as "not errors" for latency) block
		expr, err = increaseExpr()
		if err != nil {
			return monitoringv1.RuleGroup{}, err
		}
		...
		rules = append(rules, monitoringv1.Rule{
			Record: preAggBlockName(o.Indicator.Latency.Success.Name, blockWindow),
			Expr:   intstr.FromString(expr.String()),
			Labels: ruleLabelsLe,
		})
	}

	// Use block window as the evaluation interval
	return monitoringv1.RuleGroup{
		Name:     sloName + "-preagg",
		Interval: monitoringDuration(blockWindow.String()),
		Rules:    rules,
	}, nil
}
